        ...
        internal LevelThemeAsset Select() {
            if (themes == null || themes.Length == 0) {
                throw new InvalidOperationException(
                    $"[{nameof(ThemeSelectorAsset)}] No themes assigned in '{name}'."
                );
            }

            if (themes.Any(theme => theme == null)) {
                Debug.LogWarning(
                    $"[{nameof(ThemeSelectorAsset)}] Null entries detected."
                );
            }
            ...
            for (int i = 0; i < themes.Length; i++) {
                ...
                LevelThemeAsset theme = themes[idx];
                if (theme != null) { return theme; }
                if (!nullEntriesWarning) { nullEntriesWarning = true; }
            }
