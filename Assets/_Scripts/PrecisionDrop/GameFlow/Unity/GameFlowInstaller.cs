...
namespace PrecisionDrop.GameFlow.Unity {
    public sealed class GameFlowInstaller {
        ...
        public void Init() {
            if (!installed) { throw new InvalidOperationException($"[{nameof(GameFlowInstaller)}] {nameof(Init)}() called before Install()."); }
            if (initialized) {
                Debug.LogWarning($"[{nameof(GameFlowInstaller)}] Already initialized.");
                return;
            }
